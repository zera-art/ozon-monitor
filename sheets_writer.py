...
class OzonSheetsWriter:
    ...
    def _setup_rules_sheet(self):
        ...
        rows = [
            ...
            ["🔴 КРИТИЧНЫЙ ОСТАТОК",         "stock < заказов за 7д",            "Срочно пополнить остаток",               "1"],
            ["🔴 МЁРТВЫЙ ОСТАТОК",           "stock > 0 и заказов 28д = 0",      "Снизить цену −30% или вывезти",          "1"],
            ["🔴 РАСПРОДАЖА НЕЭФФЕКТИВНА",   "скидка > 20% и спрос не вырос",    "Проверить карточку, SEO, рекламу",       "1"],
            ["🟠 ЗАМЕДЛЕННАЯ ОБОРАЧИВАЕМОСТЬ","оборачиваемость > 60 дней",        "Снизить цену на 10–20%",                 "2"],
            ["🟢 НОРМАЛИЗАЦИЯ",              "спрос вырос > 20%",                "Готовиться к повышению цены",            "3"],
            ["🟢 ПОДНЯТЬ ЦЕНУ",              "оборачиваемость < 21 дня",         "Поднять цену согласно таблице",          "3"],
            ["🔵 ЦЕНА ПОДНЯТА",              "цена выросла > 5% vs прошлый период","Мониторить спрос",                    "3"],
